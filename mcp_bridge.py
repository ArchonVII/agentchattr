...
_MCP_INSTRUCTIONS = (
    ...
    "CRITICAL — Ports and Servers:\n"
    "When you open a local port (e.g. starting a dev server, database, or tunnel), you MUST announce it using chat_announce_port. "
    "This creates a special entry in chat so the human can see the address and has an easy button to kill the process when done. "
    "You must provide the port number, the purpose, and how long you expect to need it.\n\n"
...
def chat_announce_port(
    sender: str,
    port: int,
    purpose: str,
    duration: str = "until manually closed",
    address: str = "127.0.0.1",
    channel: str = "general",
    ctx: Context | None = None,
) -> str:
    """Announce that you have opened a local port (e.g. for a dev server).
    This creates a special card in chat so the human can see the address and kill it if needed.

    Args:
        sender: Your agent name
        port: The local port number (e.g. 3000, 8080)
        purpose: What the server is for (e.g. "React dev server", "Flask API")
        duration: How long you expect to need it (e.g. "5 minutes", "until task done")
        address: The host address (default: "127.0.0.1")
        channel: Channel to post in (default: 'general')
    """
    sender, err = _resolve_tool_identity(sender, ctx, field_name="sender", required=True)
    if err:
    ...
    full_address = f"http://{address}:{port}" if ":" not in address else f"{address}:{port}"
    if not full_address.startswith("http"):
        full_address = f"http://{full_address}"

    msg = store.add(
        sender, f"Opened port {port}: {purpose}",
        msg_type="port_announcement",
        channel=channel,
        metadata={
            "port": port,
    ...
    # Register in global app registry
    import app
    app.active_ports[port] = {
        "port": port,
        "sender": sender,
        "purpose": purpose,
        "duration": duration,
    ...
    with _presence_lock:
        _presence[sender] = time.time()
    return f"Port {port} announced (id={msg['id']}). address: {full_address}"
...
_ALL_TOOLS = [
    ...
    browser_open, browser_popout, chat_verify_screenshot, chat_announce_port,
]
