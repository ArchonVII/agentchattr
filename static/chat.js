...
const BRAND_AVATARS = {
  ...
  port: `<svg viewBox="0 0 16 16" fill="white"><path d="M1 8a7 7 0 1 0 14 0A7 7 0 0 0 1 8m15 0A8 8 0 1 1 0 8a8 8 0 0 1 16 0M8.5 4.5a.5.5 0 0 0-1 0v3h-3a.5.5 0 0 0 0 1h3v3a.5.5 0 0 0 1 0v-3h3a.5.5 0 0 0 0-1h-3z"/></svg>`,
};
...
function getAvatarSvg(sender) {
  ...
  if (s === "port") return BRAND_AVATARS.port;
...
function renderChannelRoster() {
  ...
  for (const entry of participants) {
    ...
    if (entry.isPort) item.classList.add("is-port");
...
function initBridgeMessageRenderer() {
  ...
  window._messageRenderers["screenshot_verification"] = function (el, msg) {
    ...
    if (disputed) {
      ...
      disputeBtn.className = "btn-dispute";
      disputeBtn.textContent = "Dispute Interpretation";
    ...
    el.innerHTML = `
      <div class="chat-bubble other" style="--bubble-color: ${senderColor}">
        <div class="bubble-header">
          <span class="msg-sender" style="color: ${senderColor}">${escapeHtml(msg.sender)}</span>
  ...
  window._messageRenderers["port_announcement"] = function (el, msg) {
    el.classList.add("port-announcement-msg");
    const meta = msg.metadata || {};
    ...
    card.innerHTML = `
      <div class="port-header">
        ...
        <span class="port-address">${escapeHtml(meta.address || "")}</span>
      </div>
      <div class="port-body">
        <div class="port-purpose"><strong>Purpose:</strong> ${escapeHtml(meta.purpose || "")}</div>
...
function initTerminalPresencePolling() {
  ...
  async function poll() {
    try {
      ...
      if (resP.ok) {
        const data = await resP.json();
        window._activePorts = data.ports || [];
      }
...
async function killPort(port, msgId) {
  if (!confirm(`Kill the process listening on port ${port}?`)) return;
  try {
    const resp = await fetch("/api/kill_port", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    // Update the card status in store
    const msgEl = document.querySelector(`.message[data-id="${msgId}"]`);
    if (msgEl) {
      const btn = msgEl.querySelector(".btn-kill-port");
      if (btn) {
        btn.disabled = true;
...
async function disputeScreenshot(msgId) {
  try {
    const resp = await fetch(`/api/messages/${msgId}/dispute_screenshot`, {
      method: "POST",
    });
    if (!resp.ok) {
      const err = await resp.json();
    ...
    console.error("Failed to dispute screenshot:", err);
  }
}
window.disputeScreenshot = disputeScreenshot;
